...
function translateCountryNameToRomanian(engName) {
  const map = {
    ...
    'southfunction getFlagEmoji(engName) {
  const map = {
...
function findCountryData(d, name) {
  // 1. Căutăm în baza de date Europeană detaliată
...
function getTerritoryById(id) {
  ...
  // 3. Căutăm în restul lumii folosind topojson data
...
function renderMap() {
  ...
  // Inițializare Proiecție bazată pe mod (2D Europa, 2D Planiglob sau 3D Glob)
  ...
  cachedCountries = svg.append('g')
    ...
    .attr('fill', d => {
      ...
      const countryData = findCountryData(d, name);
      if (countryData) {
        return `var(--${countryData.categorie})`;
      }
      return 'var(--land)'; // Default fallback land color
    })
...
function updateMarkerPositions() {
  ...
  cachedMarkers.each(function(t) {
    const isVisible = currentMode.startsWith('2d') || isVisibleOnGlobe(t.coords);
    const projected = projection(t.coords);
...
document.getElementById('zoom-reset').addEventListener('click', () => {
  if (currentMode.startsWith('2d')) {
    ...
    const rect = container.getBoundingClientRect();
    projection.scale(Math.min(rect.width, rect.height) * 0.45).rotate([-rotationState[0], -rotationState[1], rotationState[2]]);
    updateProjection();
  }
});
...
function getGlobalSearchList() {
  ...
  const list = [...teritorii];
  
  // Adăugăm marile puteri
  for (const name in globalMajorData) {
    const raw = globalMajorData[name];
    if (raw.inherits) continue;
  ...
  // Adăugăm dinamic toate celelalte țări din world-data
  const worldDataNode = document.getElementById('world-data');
  if (worldDataNode) {
    const worldData = JSON.parse(worldDataNode.textContent);
    const countries = topojson.feature(worldData, worldData.objects.countries);
    countries.features.forEach(f => {
      ...
      if (!list.some(x => x.id === id)) {
        const countryData = findCountryData(f, name);
        if (countryData) {
